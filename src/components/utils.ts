...
export const entries: (
  obj: {},
) => Array<[string, any]> = Object.entries.bind(Object);
export const run: (fn: any) => void = fn => fn();

export const normalizer: <T>(
  handler: any,
) => (acc: T[], arr: T[]) => T[] = handler => (acc, arr) =>
  acc.concat(handler(arr));
...
export const formatSwaggerPath: (
  path: string,
) => string = normalizeUrlParams(/:(\w+)/g, '{$1}');

export const formatRouterPath: (
  path: string,
) => string = normalizeUrlParams(/{(\w+)}/g, ':$1');

export const objectPath: (object: {}, path: string) => any = (
  object,
  path,
) =>
  // @ts-ignore
